...
function displayCart() {
  ...
  cart.forEach((item, index) => {
    ...
    cartItem.innerHTML = `
          ...
          <div class="quantity"><input type="number" name="" value="${item.quantity}" min="1" data-index="${index}"></div>
          <span class="total-price">ZAR ${itemTotal}</span>
...
`;
  ...
  subtotalEl.textContent = `ZAR ${subtotal.toFixed(2)}`;
  grandTotalEl.textContent = `ZAR ${subtotal.toFixed(2)}`;
